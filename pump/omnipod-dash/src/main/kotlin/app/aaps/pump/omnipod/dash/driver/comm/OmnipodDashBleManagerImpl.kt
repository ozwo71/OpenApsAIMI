...
import app.aaps.pump.omnipod.dash.driver.comm.exceptions.*
...
import app.aaps.pump.omnipod.dash.driver.comm.session.*
...
class OmnipodDashBleManagerImpl @Inject constructor(
    ...
    private fun acquireBusyFlag(): Boolean {
        ...
        while (!busy.compareAndSet(false, true)) {
            if (System.currentTimeMillis() - start > MAX_WAIT_MS) {
                forceResetGatt()
                if (busy.compareAndSet(false, true)) break
            }
            Thread.sleep(100)
        }
        return true
    ...
    private fun forceResetGatt() {
        try {
            ...
            aapsLogger.error(LTag.PUMPBTCOMM, "Erreur lors du reset BLE: $t")
        } finally {
            busy.set(false)
        }
    }
    ...
    override fun sendCommand(cmd: Command, responseType: KClass<out Response>): Observable<PodEvent> =
        Observable.create { emitter ->
            if (!acquireBusyFlag()) {
                emitter.tryOnError(BusyException())
                return@create
            }
            try {
    ...
    private fun connect(connectionWaitCond: ConnectionWaitCondition): Observable<PodEvent> =
        Observable.create { emitter ->
            ...
            try {
                ...
                val podAddress =
                    podState.bluetoothAddress ?: throw FailedToConnectException("Missing bluetoothAddress, activate the pod first")
    ...
    private fun assertPaired(): ByteArray {
        return podState.ltk ?: throw FailedToConnectException("Missing LTK, activate the pod first")
    }
    ...
    override fun pairNewPod(): Observable<PodEvent> =
        Observable.create { emitter ->
            ...
            try {
                ...
                val ltkExchanger = LTKExchanger(aapsLogger, config, mIO, ids)
    ...
    companion object {
        const val CONTROLLER_ID = 4242 // TODO : lire depuis les préférences ou un autre emplacement
    }
}
