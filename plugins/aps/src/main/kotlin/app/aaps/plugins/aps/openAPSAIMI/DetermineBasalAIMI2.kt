...
// ⚠ Alignement avec `maxSMB` et `profile.peakTime`
        ...
        if (isMealModeCondition2(variableSensitivity, targetBg, delta, shortAvgDelta, autodrive, mealData.slopeFromMinDeviation, bg.toFloat()) && !mealTime && !highCarbTime && !lunchTime && !bfastTime && !dinnerTime && !snackTime && bgAcceleration.toFloat() >= 2.0f){
            val pbolusM: Double = preferences.get(DoubleKey.OApsAIMIMealPrebolus)
            rT.units = pbolusM
            rT.reason.append("Microbolusing Meal Mode ${pbolusM}U. ")
            return rT
...
// 🔹 Sécurisation des bornes minimales et maximales
        ...
        val logTemplate = buildString {
            ...
            appendLine(String.format("║ %-${screenWidth}s ║", "22 Feb 2025"))
            appendLine("╚${"═".repeat(screenWidth)}╝")
            appendLine()
            ...
            var rate = profile_current_basal
                ...
                if (honeymoon) {
                    when {
                        ...
                        bg > 170 && delta > 0 -> rate = calculateBasalRate(basalaimi.toDouble(), profile_current_basal, basalAdjustmentFactor)
