...
class LegacyPrebolusMissedDeliveryTest {
    ...
    @Test
    fun `tire mais delai de confirmation pas ecoule - pas d'alerte (protege du retard cache)`() {
        // Tir il y a 5 min < 6,5 min de délai : le cache SMB asynchrone peut encore être en retard.
    ...
    fun `tire, delai ecoule, aucun SMB depuis - alerte`() {
        // P1 tiré à runtime 0, ré-évaluation à runtime 8 (MAINT), aucun SMB en base depuis.
        val firedAt = now - min(8)
        assertTrue(DetermineBasalaimiSMB2.legacyPrebolusMissedDelivery(firedAt, null, now, 8))
        // Idem avec un SMB confirmé ANTÉRIEUR au tir (vieux bolus d'un repas précédent).
        assertTrue(DetermineBasalaimiSMB2.legacyPrebolusMissedDelivery(firedAt, firedAt - min(120), now, 8))
    }
    ...
    fun `tire et SMB confirme apres le tir - pas d'alerte (delivrance prouvee)`() {
        ...
        assertFalse(DetermineBasalaimiSMB2.legacyPrebolusMissedDelivery(firedAt, smbConfirmed, now, 8))
    }
    ...
    fun `borne - detection possible juste apres le delai de confirmation`() {
        ...
        assertFalse(DetermineBasalaimiSMB2.legacyPrebolusMissedDelivery(firedAt + 1, null, now, 8))
    }
}
