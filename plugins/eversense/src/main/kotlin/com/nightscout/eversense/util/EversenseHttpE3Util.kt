...
class EversenseHttpE3Util {
    companion object {
        ...
        // EU/OUS endpoints ù confirmed from decompiled Eversense EU app v7.1.1
        ...
        fun login(preferences: SharedPreferences): EversenseHttp365Util.LoginResponseModel? {
            ...
            return try {
                ...
                if (code >= 400) {
                    ...
                    EversenseLogger.error(TAG, "E3 login failed ù status: $code, body: $err")
                    return null
                }
                val body = conn.inputStream.readBytes().toString(Charsets.UTF_8)
                EversenseLogger.info(TAG, "E3 login success ù status: $code")
                Json.decodeFromString(EversenseHttp365Util.LoginResponseModel.serializer(), body)
            } catch (e: Exception) {
        ...
        /**
         * POST api/care/PutCurrentValues ù updates Last Sync Date on portal and feeds AGP reports.
         */
        fun putCurrentValues(
            ...
            val token = getOrRefreshToken(preferences) ?: run {
                EversenseLogger.error(TAG, "E3 putCurrentValues ù no valid token")
                return false
            }
            return try {
                ...
                if (code >= 400) {
                    val err = conn.errorStream?.readBytes()?.toString(Charsets.UTF_8) ?: ""
                    EversenseLogger.error(TAG, "E3 PutCurrentValues failed ù status: $code, body: $err")
                    false
                } else {
                    EversenseLogger.info(TAG, "E3 PutCurrentValues ok ù status: $code, glucose=$glucose")
                    true
                }
            } catch (e: Exception) {
        ...
        /**
         * POST api/care/PutDeviceEvents ù populates Sensor Glucose history table on portal.
         */
        fun putDeviceEvents(
            preferences: SharedPreferences,
            ...
            val token = getOrRefreshToken(preferences) ?: run {
                EversenseLogger.error(TAG, "E3 putDeviceEvents ù no valid token")
                return false
            }
            return try {
                ...
                if (code >= 400) {
                    val err = conn.errorStream?.readBytes()?.toString(Charsets.UTF_8) ?: ""
                    EversenseLogger.error(TAG, "E3 PutDeviceEvents failed ù status: $code, body: $err")
                    false
                } else {
                    EversenseLogger.info(TAG, "E3 PutDeviceEvents ok ù status: $code, readings: ${readings.size}")
                    true
                }
            } catch (e: Exception) {
